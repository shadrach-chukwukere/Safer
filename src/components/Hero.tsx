...
const imageScaleVariants: Variants = {
  ...
  visible: { scale: 1, opacity: 1, transition: { duration: 0.5, stiffness: 260, damping: 20 } },
};

const infoBoxVariants: Variants = {
  hidden: { opacity: 0, scale: 0.8 },
  visible: { opacity: 1, scale: 1, transition: { type: "tween", duration: 0.5, delay: 0.5 } },
};
...
export default function Hero() {
  ...
  const totalImages = 3; // Number of images in the hero section

  useEffect(() => {
    if (imagesLoaded === totalImages) {
      // Wait just a bit to ensure smooth animation after all images load
  ...
  return (
    <div className={`bg-black text-white py-16 md:py-32 font-Inter min-h-screen transition-opacity duration-700 ${loaded ? "opacity-100" : "opacity-0"}`}>
      <div className="grid grid-cols-1 md:grid-cols-2 justify-items-center mx-10 items-center gap-12">
        ...
        >
          <motion.h1 className="font-extrabold leading-[1.1] text-center" variants={textItemVariants}>
            <span className="block text-[42px] sm:text-[50px] lg:text-[75px]">Experience Ai</span>
            <span className="block text-[28px] sm:text-[40px] lg:text-[50px]">powered payments</span>
          </motion.h1>

          <motion.p className="text-sm sm:text-md md:text-lg font-semibold max-w-lg mx-auto md:mx-0 text-center" variants={textItemVariants}>
            Receive, send, convert, send funds locally, create and make purchases with customized USD virtual cards.
          </motion.p>

          <motion.div className="pt-6 flex justify-center lg:justify-start" variants={textItemVariants}>
            <GetSaferBtn />
          </motion.div>
        </motion.div>
        ...
        <div className="flex flex-col items-center relative sm:gap-2 gap-4 relative">
          ...
          >
            <span className="font-[inter]">
              Safer is changing the payment landscape with smart options for payment, join safer today...
            </span>
          </motion.div>

          <div className="flex justify-center gap-4 sm:gap-8 mt-6">
            {[image3, image1].map((imgSrc, i) => (
              <motion.div
                key={i}
